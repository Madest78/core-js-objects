...
function makeWord(lettersObject) {
  const maxPosition = Object.values(lettersObject).reduce((max, positions) => {
    const maxPos = Math.max(...positions);
    return maxPos > max ? maxPos : max;
  }, -1);
  ...
  function addLetters(letter) {
    if (lettersObject[letter].includes(i)) {
      word += letter;
    }
  }

  for (i = 0; i <= maxPosition; i += 1) {
    Object.keys(lettersObject).forEach(addLetters);
  }
